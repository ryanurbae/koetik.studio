...
export default function GalleryView({
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${title.replace(/[^a-zA-Z0-9 -]/g, "")}_koetikstudio.zip`;
      a.click();
  ...
  return (
    <div
      className={`${spaceGrotesk.variable} ${inter.variable} min-h-screen bg-[#0a0a0a] text-white font-body scroll-smooth`}
    >
      {/* Header */}
      <motion.header
        ...
        className="max-w-5xl mx-auto px-4 sm:px-8 pt-14 pb-8 sm:pt-20 sm:pb-12 text-center"
      >
        <p className="text-[10px] uppercase tracking-[0.3em] text-white/30 font-medium mb-4">
          [ gallery ]
        </p>
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-heading font-bold tracking-tight leading-[1.05] mb-4">
          {title}
        </h1>
        {description && (
        ...
        <div className="mt-8 h-px bg-white/[0.06] max-w-xs mx-auto" />
      </motion.header>
      ...
      <motion.main
        ...
        className="max-w-5xl mx-auto px-4 sm:px-8 py-8 sm:py-12"
      >
        {photos.length > 0 ? (
          <div className="columns-2 sm:columns-3 lg:columns-4 gap-2 sm:gap-3">
            {photos.map((photo, index) => (
              ...
              >
                <div className="relative overflow-hidden rounded-lg ring-1 ring-white/[0.05] transition-all duration-300 group-hover:ring-white/20 group-hover:shadow-xl group-hover:shadow-black/40">
      ...
      <motion.footer
        ...
        className="max-w-5xl mx-auto px-4 sm:px-8 py-10 border-t border-white/[0.04]"
      >
        <div className="flex flex-col items-center gap-4">
          <p className="text-xs text-white/20">{photos.length} photos</p>
          {photos.length > 0 && (
            <button
              onClick={handleDownloadAll}
              disabled={downloading}
              className="px-6 py-2.5 rounded-lg bg-white/[0.06] ring-1 ring-white/[0.08] text-xs font-medium text-white/50 hover:bg-white/[0.1] hover:text-white transition-all disabled:opacity-50"
            >
              {downloading ? `Downloading ${downloadProgress}...` : "Download All"}
            </button>
          )}
          <a
            href="https://koetikstudio.my.id"
            target="_blank"
            ...
            >
              <AnimatePresence mode="wait" custom={lightboxDirection}>
                <motion.img
                  ...
                  initial={{ opacity: 0, x: lightboxDirection * 60, scale: 0.97 }}
                  animate={{ opacity: 1, x: 0, scale: 1 }}
                  exit={{ opacity: 0, x: lightboxDirection * -60, scale: 0.97 }}
